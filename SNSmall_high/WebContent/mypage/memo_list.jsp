...
if(count!=0){
	...
	if(startPage>pageBlock){ // 스타트페이지가 페이지블럭보다 많을 때
		%><li><a href="MemoList.me?pageNum=<%=startPage-pageBlock%>">&laquo;</a></li>&nbsp;<%
	}
	//1...10   11...20   21...30
	for(int i=startPage; i<=endPage; i++){
		if(i==currentPage){%>
                    ...
                    <%}else{ %>
                    <li><a href="MemoList.me?pageNum=<%=i%>">[<%=i%>]</a></li>
                    <%}}
	//다음 페이지
	if(endPage<pageCount){ //엔트페이지가 페이지블록보다 작을 때
		%><li><a href="MemoList.me?pageNum=<%=startPage+pageBlock%>">&raquo;</a></li><%
	}
}
