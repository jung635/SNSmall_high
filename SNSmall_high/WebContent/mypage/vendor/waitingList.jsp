...
List<PaymentBean> waiting_list = (List<PaymentBean>)request.getAttribute("list");
        ...
        <div class="row">
            ...
            <h2>취소 목록</h2>
            	...
            	<%}else{ %>
                  ...
                  <tr><th>사진</th><th>이름</th><th>상품 정보</th><th>주문번호</th><th>배송상태</th><th>주문일</th><th>취소</th></tr>
                  <%
                  for(int i=0; i<waiting_list.size(); i++){
                	  PaymentBean pb = waiting_list.get(i);
                  ...
                  <td><%=pb.getState() %></td>
                  <td><%=pb.getDate() %></td>
                  <td><%=pb.getUsedPoint() %></td>
                  <td><input type="button" value="디파짓 OK" onclick="location.href='PayDepositDone.ve?num=<%=pb.getNum()%>'"></td>
                  </tr>
                  <%} %>
                  </table>
                  <%} %>
